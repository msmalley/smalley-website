...
const APPLY_URL = 'https://hrhub.wd3.myworkdayjobs.com/Euronext_Career_Page/job/Paris/Chief-Product-Officer--Digital-Assets_R27756/apply/autofillWithResume';

const wait = ms => new Promise(r => setTimeout(r, ms));

async function shot(page, label) {
  const file = `${SCREENSHOTS}/euronext_${label}.png`;
  try {
...
async function fillMyInformation(page) {
  ...
  // 1. How Did You Hear - button#source--source opens a listbox
  const sourceBtn = await page.$('#source--source');
  if (sourceBtn) {
    const currentVal = await page.evaluate(el => el.textContent.trim(), sourceBtn);
    if (currentVal.includes('LinkedIn')) {
      console.log('  How Heard: already set to', currentVal);
    } else {
      await sourceBtn.click();
      await wait(2000);
      // Options are role="option" li items with plain IDs (not promptOption)
      const selected = await page.evaluate(() => {
        const opts = [...document.querySelectorAll('[role="option"]')];
        const linkedin = opts.find(el => el.textContent.trim() === 'LinkedIn');
        if (linkedin) { linkedin.click(); return linkedin.textContent.trim(); }
        return null;
      });
      if (selected) {
        console.log('  How Heard: selected', selected);
      } else {
        console.log('  How Heard: FAILED - no LinkedIn option found');
      }
      await wait(500);
    }
  } else {
    console.log('  How Heard: button not found, skipping');
  }
